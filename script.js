...
document.body.addEventListener("click", () => {
    ...
    if (messageIndex < messages.length) {
        ...
        switch (clickCount) {
                ...
                // 3초 뒤 이미지 흩어지며 사라지게 하기
                setTimeout(() => {
                    img.style.opacity = 0; // 투명도 0으로 설정
                    // 흩어지듯 사라지기: 여러 방향으로 이동 및 회전
                    const randomXOffset = (Math.random() - 0.5) * 100; // -50% ~ +50% 화면 너비
                    const randomYOffset = (Math.random() - 0.5) * 100; // -50% ~ +50% 화면 높이
                    const randomRotation = (Math.random() - 0.5) * 360; // 360도 회전

                    img.style.transform = `scale(0.8) translate(${randomXOffset}vw, ${randomYOffset}vh) rotate(${randomRotation}deg)`; // 축소, 이동, 회전

                    // DOM에서 제거
                    setTimeout(() => {
                        img.remove();
                    }, 1000); // 사라지는 애니메이션이 끝난 뒤 DOM에서 제거
                }, 3000 + index * 500); // 각 이미지의 딜레이를 고려하여 3초 뒤 시작
            });
...
async function processImages() {
    ...
    for (let x = 0; x < img1.width; x += pixelSize) {
        for (let y = 0; y < img1.height; y += pixelSize) {
            ...
            // 색상 추출 및 픽셀 위치 매핑
            let depth = map(pixel1[0], 0, 255, -100, 100); // 첫 번째 이미지로 깊이 설정
            pixelsData.push({
...
function initThreeJS() {
    ...
    pixelsData.forEach(pixel => {
        ...
        // 6면 중 하나는 두 번째 이미지 색상, 나머지는 첫 번째 이미지 색상
        const materials = [
            new THREE.MeshBasicMaterial({ color: pixel.color1 }), // right
